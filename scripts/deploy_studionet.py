#!/usr/bin/env python3
"""Deploy contracts/agentguard.py to GenLayer StudioNet and record the
...
- refuses to deploy unless contracts/agentguard.py is committed and
...
def main() -> None:
    ...
    if git("status", "--porcelain", "--", "contracts/agentguard.py"):
        raise SystemExit("refusing: contracts/agentguard.py has uncommitted changes")
    commit = git("log", "-1", "--format=%H", "--", "contracts/agentguard.py")
    head = git("rev-parse", "HEAD")
    blob = git("rev-parse", "HEAD:contracts/agentguard.py")
