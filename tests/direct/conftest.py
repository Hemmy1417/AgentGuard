"""Direct Mode harness for contracts/agentguard.py (see support.py for what
...
from tests.direct.support import CONTRACT, MODULE, NOW, setup
...
def _tolerate_windows_unlink():
    ...
    if getattr(original, "_agentguard_shim", False):
        return
    ...
    inject_tolerant._agentguard_shim = True
...
@pytest.fixture
def guard(direct_vm, direct_deploy):
...
@pytest.fixture
def genlayer_vm(guard):
    import genlayer
    return genlayer.gl.vm


@pytest.fixture
def policy_id(guard, direct_vm):
    """Both agents registered and a settlement policy published."""
    return setup(guard, direct_vm)
